...
.. note::
   ...
   static, manually input data or a range of Odoo data that had been inserted into the spreadsheet,
   e.g., from an Odoo list view.
...
To be able to :ref:`manipulate the dimensions and measures
<spreadsheet/pivot-tables/build-manipulate>` of a pivot table that has been inserted from an Odoo
pivot view, it first has to be converted to a :ref:`dynamic pivot table
...
- **Re-insert the dynamic pivot table from the Data menu**: On the sheet that contains your static
  ...
  .. tip::
     It is also possible re-insert the dynamic pivot table by entering the :ref:`function of the
...
- `include_column_titles` and `include_measure_titles`: set value of `0` to remove the column and
...
- **Use the 'Convert to individual formulas' option**: Right-click on any cell in the pivot table,
  then click :guilabel:`Convert to individual formulas`. Every populated cells is replaced by an
  individual function; click on a cell to see the function in the formula bar.
...
retrieves data from your your database once :ref:`columns, rows, and measures have been added
