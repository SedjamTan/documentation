...
- :guilabel:`Registered for Working Holiday Maker`: Tick this box if the company is registered for
  ...
  |ATO| to withhold the correct taxes for employees working in Australia with an eligible visa, for
...
- Work entries are only generated after clocking in and out of the **Attendances** app. Note that
  attendances can be imported.
- Work entries are generated from planning shifts in the **Planning** app.

.. important::
   Timesheets do not impact work entries in Odoo. If timesheets need to be imported in Odoo, import
   them by navigatig to :menuselection:`Payroll app --> Work Entries --> Work Entries` instead.
...
- :guilabel:`Wage`: Enter the wage, and select the corresponding time period. The following options
  are the **only** pay run frequencies accepted byAustralia: :guilabel:`Daily`, :guilabel:`Weekly`,
...
- :guilabel:`TFN Status`: Using the drop-down menu, select the eployee's declaration.
...
- :guilabel:`Eligible for Leave Loading`: Check if the employee is eligible for leabe loading.
...
Scroll ot the *Australian Localization* section, and under *SuperStream and Single Touch Payroll*,
...
To do so, after importing YTD balances, nabigate to :menuselection:`Payroll app --> Reporting -->
...
- **W3**: Other amounts withheld (excluding any amount shown at W2 or W4) To use it tick the box
  *Report in W3* in the employee's record, in the *Payroll* tab.
...
- *What one of the fund payment fails?*
...
- :guilabel:`Unused Leave Type`: Choose between :guilabel:`Annual`, :guilabel:`Long Service`, or
  ...
  leaves of the type :guilabel:`Annual` will include leave loading if the employee is eligible.
...
.. note::
   Annual leaves will be taxed as per as per |ATO| schedule 7. ETPs will be taxed as per |ATO|
   schedule 11.
