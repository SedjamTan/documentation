...
The Hungarian localization package offers key features and integration capabilities that ensures
...
#. To configure NAV 3.0, go to the `government's portal <https://onlineszamla.nav.gov.hu/home>`_ and
   log in using your **Ügyfélkapu+** details.
#. Select 'Primary User' and log in via the KAÜ system. Select the correct taxpayer profile if
   prompted.
#. Go to the 'Users' menu, click 'New User', and select 'Technical User'. *(This
   type is required for machine integrations like Odoo).*
#. Set a username and password, grant permissions for 'Manage invoices' and 'Query invoices', and
   click 'Save'.
#. On the users details page, click 'Generate Key'. Copy and save the **Username**, **Password**,
   **XML Signing Key**, and **XML Cryptographic Key**.
...
#. Scroll to the :guilabel:`Hungarian Electronic Invoicing` section and select the desired
   :guilabel:`Mode` (:guilabel:`Production`, :guilabel:`Test`, or :guilabel:`Demo`).
...
#. Fill in the :guilabel:`Company Name`, :guilabel:`VAT` (TIN), and :guilabel:`Country`. If desired,
   ...
   .. Important::
      - The :guilabel:`Country` must be set to :guilabel:`Hungary`. NAV 3.0 requires a valid
        :guilabel:`City`, :guilabel:`Street`, and :guilabel:`Zip`.
      - The :guilabel:`Company Name` must match the name registered with the National Tax and
