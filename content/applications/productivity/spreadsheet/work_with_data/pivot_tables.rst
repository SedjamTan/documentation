...
Pivot tables allow you to organize, summarize and analyze large amounts of data without the need for
complex formulas. By grouping and segmenting data into
:ref:`dimensions <spreadsheet/pivot-tables/dimensions>` (i.e., the fields added as columns and
rows) and defining what is being :ref:`measured <spreadsheet/pivot-tables/measures>` (e.g., the
total amount or quantity ordered), the corresponding values of each intersecting data point are
...
- :ref:`insert a pivot table referencing Odoo data directly from an Odoo spreadsheet
  <spreadsheet/pivot-tables/create>`. This option allows you to retrieve and work with data from
  any model, including models for which no Odoo pivot view is available e.g., the *Sales Order Line*
  model.
...
.. tip::
   ...
   - format your data as a table by selecting any cell in the range, clicking :icon:`os-edit-table`
     :guilabel:`Insert table` from the toolbar, then selecting a style. This ensures any updates to
     the table are reflected in the pivot table
...
The :guilabel:`Columns` and :guilabel:`Rows` are the :ref:`dimensions
<spreadsheet/pivot-tables/dimensions>` used to group or segment data, while the :ref:`Measures
<spreadsheet/pivot-tables/measures>` define what is being measured, or analyzed, based on the
selected dimensions.
...
Adding multiple row or columns dimensions creates a nested hierarchy. To change the hierarchy
of the dimensions, drag the dimension's card to the desired position within its.
...
#. Click :icon:`fa-exchange` :guilabel:`Flip axes`.
  ...
  .. note::
     ...
     something is preventing cells being populated, such as other data, merged cells, or the
...
#. Right-click one of the selected cells, then click :icon:`fa-plus-square-o` :guilabel:`Group
   ...
   section, as relevant. Change the sorting to :guilabel:`Ascending` or :guilabel:`Descending`.gi
...
.. example::
   In the example, data per salesperson for the secondary markets of Spain, France, Ireland and
   Italy has been grouped into an :guilabel:`Others` category, which can then be collapsed and its
   data seen beside that of the main markets of Belgium and the United States.
...
#. Optionally, edit the name of the measure by clicking then editing the measure's name.
...
.. note::
   The fields needed for the calculation of the new measure must already been added to the pivot
   table as dimensions or measures.
...
#. Open the :ref:`pivot table's properties panel <spreadsheet/pivot-tables/properties>`.
#. On the measure's card, click the :icon:`fa-cog` :guilabel:`Show values as` icon.
...
.. tip::
   In a static pivot table inserted from an Odoo pivot view, delete the second header row manually,
   if desired.
...
.. note::
   ...
   - When a pivot table is duplicated, the new pivot table is by default a :ref:`dynamic pivot table
     <spreadsheet/insert-pivot-table/static-vs-dynamic>` whose dimensions and measures :ref:`can be
     manipulated <spreadsheet/pivot-tables/build-manipulate>`.
...
.. note::
   ...
   source in the :guilabel:`Data` menu, while deleting the data source first results in pivot table
   filled with :guilabel:`#ERROR` errors.
