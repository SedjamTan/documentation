...
valuation only when specifically requested, most often for closing or in perpetual accounting, when
vendor bills or customer invoices are posted.
...
- :ref:`Valuation Account <accounting/inventory-valuation/valuation-account>`: Set the asset account
  ...
  :guilabel:`(right arrow)` in the :guilabel:`Valuation Account` field to open the :guilabel:`Stock
  Valuation` account and update the :ref:`Variation Account
...
- :guilabel:`Other accounts can be defined on "Inventory Loss" and "Production" on dedicated
  locations`: If needed, set the :ref:`Inventory Loss
...
- :guilabel:`Periodic (at closing)` updates inventory valuation only when generating entries during
  ...
  is debited when vendor bills are posted.
- :guilabel:`Perpetual (at invoicing)` updates inventory valuation when bills or invoices are
  posted. The Expenses account (:abbr:`COGS (Cost of goods sold)`) is debited when invoices are
  posted. Combined with the :ref:`stock closing process
  <accounting/inventory-valuation/closing-entry>`, it updates inventory value for deliveries and
  receipts, even if there is no invoice or vendor bill yet.
...
.. note::
   ...
   different accounting valuation method based on their specific needs.

.. tip::
   When using :guilabel:`Perpetual (at invoicing)` valuation method with the :guilabel:`Standard
   Price` costing method, a :guilabel:`Price Difference Account` can be set in the
...
- Anglo-Saxon :guilabel:`Perpetual (at invoicing)`: Current asset account (recommended for interim
  ...
  .. note::
     ...
     - In perpetual accounting, if no bill or invoice has been posted for these moves yet, it allows
       their impact on inventory value to be recorded in the accounting. The :guilabel:`Variation
       account` serves as a buffer account until the :ref:`accrual entries
       <accounting/inventory-valuation/accrual-entries>` are recorded.
...
account (:guilabel:`Inventory Loss` location type):

#. In the :guilabel:`Other accounts can be defined on "Inventory Loss" and "Production" on dedicated
   locations` :ref:`configuration <accounting/inventory-valuation/configuration>`, click
   :guilabel:`locations`.
...
.. note::
   - In Continental :guilabel:`Periodic (at closing)`, the update is displayed in the
     :guilabel:`Inventory Loss` section of the :ref:`Inventory Valuation report
     <accounting/inventory-valuation/inventory-valuation-report>`.
   - In Anglo-Saxon :guilabel:`Perpetual (at invoicing)`, the :ref:`valuation account
...
To record production costs in a different account than the general set :guilabel:`Variation
account`, for both the finished product and the components, set up a :guilabel:`Production` location
account (:guilabel:`Production` location type):
...
.. note::
   - In Anglo-Saxon :guilabel:`Perpetual (at invoicing)`, the :guilabel:`Valuation account` for
     finished products and components are updated when a manufacturing order is validated, and the
     :guilabel:`Cost of Production` account serves as the counterpart account for both.
   - In Continental :guilabel:`Periodic (at closing)`, inventory variations from production (for
     both finished products and components) are recorded separately from general inventory
     variations recorded at closing, isolating production-related changes from receipts and
     deliveries. However, this account is not commonly used in standard Continental
     :guilabel:`Periodic (at closing)` accounting.
...
- :guilabel:`Initial Balance`: Current accounting inventory value. Click to display the stock
...
- :guilabel:`Cost of Production` (:guilabel:`Periodic (at closing)` only, even if using this account
  is not common for this accounting method): Only if the :ref:`Cost of Production
...
- :guilabel:`Ending Stock`: Future accounting inventory value when the :ref:`closing entry
  <accounting/inventory-valuation/closing-entry>` is completed. Click to access the :doc:`Inventory
  Stock <../../../inventory_and_mrp/inventory/warehouses_storage/reporting/stock>` report.
...
.. note::
   Both valuation methods use accrual entries to update revenues and expenses. However, in
   Anglo-Saxon :guilabel:`Perpetual (at invoicing)`, these entries also take value from the buffer
   :guilabel:`Variation account` and allocate it to the appropriate accounts, such as Bill to
   Receive or GRNI.
...
<accounting/inventory-valuation/valuation-account>` via manual journal entry to maintain accurate
inventory values after migration. This rebalancing can be performed :ref:`before
...
.. important::
   It’s recommended first to use a testing database created during the upgrade process and verify
   the :doc:`Stock report
...
After upgrading to Odoo 19, the server action must be applied per company to balance any non-zero
:guilabel:`Stock Interim` accounts:
...
#. (optional) Generate :ref:`accrual entries <accounting/inventory-valuation/accrual-entries>` if an
   accounting closing is required and the Bill to Receive or GRNI account, etc., need updating.
...
#. After upgrading to Odoo 19, go to :menuselection:`Accounting --> Review --> Inventory Valuation`
   ...
   account.
#. (optional) Generate :ref:`accrual entries <accounting/inventory-valuation/accrual-entries>`
