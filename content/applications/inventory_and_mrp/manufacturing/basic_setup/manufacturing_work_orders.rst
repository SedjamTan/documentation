...
.. note::
   If the product does not have a |BoM| configured, a |BoM| can be generated based on the current
   |MO|.
   ...
   :guilabel:`Reference` field filled in with *New BoM from WH/MO/<MO number>*.

   Click the :icon:`fa-cloud-upload fa-fw` :guilabel:`(Save manually)` icon to create the new |BoM|.
   After saving, the :guilabel:`BoM Type` is set to **Manufacture this product** and the
