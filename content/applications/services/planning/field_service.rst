...
<planning/field-service/performing-shift>` and billed to the customer. A customer report can also
be generated at the end of the intervention for signature, and :ref:`itineraries
<planning/field-service/itinerary>` can be planned for field employees.

.. _planning/field-service/configuration:

Configuration
=============

To enable the feature, go to :menuselection:`Planning --> Configuration --> Settings`, enable
:guilabel:`Field Service`, and click :guilabel:`Save`. Once enabled, the following options become
available in the :guilabel:`Field Service` section:

- :guilabel:`Billing`: to invoice your time and materials to your customers. xxx installs sales!!

- :guilabel:`Products`: if :guilabel:`Billing` is enabled, automatically enabled to track the materials used during interventions and bill them to the
  customer.

- :guilabel:`Customer Report`: if billing is enabled, automatically enabled to provide the customer with an intervention report, collect their
...
.. _planning/field-service/performing-shift:
...
Worksheets
==========
xxx then mention Worksheet tab in shifts
...
.. _planning/field-service/worksheet-templates:
